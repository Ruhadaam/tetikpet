...
import { createDispatch } from "@/actions/data.actions";
...
export function DispatchForm({
  ...
  const isFormValid =
    formData.product_id && formData.customer_id && Number(formData.quantity) > 0 && date;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, quantity: "", customer_id: "" }));
      setDate(new Date());
    } catch (error: any) {
      ...
      <GlassCard className="border border-input shadow-sm rounded-lg overflow-visible bg-white/50 hover:bg-white/50 hover:shadow-sm">
        <form onSubmit={onSubmit} className="p-6 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <SelectField
                ...
                placeholder="Çıkışı yapılacak ürünü seçin..."
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2 flex flex-col">
                <label className="text-[10px] font-black text-muted-foreground/50 px-0.5 flex items-center gap-2 uppercase tracking-[0.15em] mb-1">
                  <CalendarIcon className="h-3.5 w-3.5" /> Sevkiyat Tarihi
                </label>
                <Popover>
          ...
          <div className="pt-4 flex justify-end">
            ...
            >
              <Send className="mr-2 h-5 w-5" /> SEVKİYATI TAMAMLA
            </LoadingButton>
          </div>
        </form>
      </GlassCard>
